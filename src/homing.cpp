...
{
    int operation_mode = 0x06; // homing first non sense command
    ...
    try
    {
        ...
        for (std::vector<minas_control::MinasClient *>::iterator it = clients.begin(); it != clients.end(); ++it)
        {
            ...
            printf("operational mode is: %d\n",operation_mode);
            // output.operation_mode = 0x08; // (csp) cyclic synchronous position mode
            // output.operation_mode = 0x06; // (hm) homing mode
            ...
            else if (operation_mode == 0x06)
            {
...

                {
                    ...
                    printf("bit 10: %d (target reached) bit 12: %d (homing attained) bit 13: %d (homing error)\n", bit10, bit12, bit13);
                }
            }
        }
